...
export class GroceryView extends View {
	...
	<div class="p-m">
		<div>
			...
			<vaadin-number-field ${field(this.binder.model.quantity)} has-controls label="Quantity"></vaadin-number-field>
			<vaadin-button theme="primary" @click=${this.addItem} ?disabled=${this.binder.invalid}>Add</vaadin-button>
		</div>
	
		<h3>Grocery List</h3>
		<vaadin-grid .items="${this.groceryList}" theme="row-stripes" style="max-width: 400px">
	...
	async addItem() {
		...
		if (groceryItem) {
			this.groceryList = [...this.groceryList, groceryItem];
			this.binder.clear();
		}
	}
